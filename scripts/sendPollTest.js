  ...
  try {
    ...
    for (const d of dirs) {
      ...
      try {
        ...
        console.log('Found DevToolsActivePort at', devtoolsFile, 'port', port, '-> fetching', url);
        const res = await fetch(url);
        const json = await res.json();
        console.log('DevTools /json/version result keys:', Object.keys(json || {}));
        if (json && json.webSocketDebuggerUrl) return json.webSocketDebuggerUrl;
      } catch (e) {
        // try next session folder
        console.debug('No DevToolsActivePort at', devtoolsFile, ':', e && e.message);
        continue;
      }
    }
    return null;
  } catch (e) {
    console.debug('No existing DevTools websocket found (error):', e && e.message);
    return null;
  }
}
...
async function createClient() {
  ...
  if (ws) {
    ...
    return new Client({ authStrategy: auth, puppeteer: { browserWSEndpoint: ws } });
  }
...
client.on('ready', async () => {
  ...
  try {
    ...
    console.error('Erro ao enviar mensagem pré-poll:', (e && e.message) || e);
  }
  ...
  try {
    ...
    try {
      ...
      msgs.forEach(m => console.log('-', m.id._serialized, m.type, m.body || '(no body)'));
    } catch (e) {
      console.error('Erro ao buscar chat/mensagens:', (e && e.message) || e);
    }
  } catch (err) {
    console.error('Erro ao enviar poll:', err);
  }
