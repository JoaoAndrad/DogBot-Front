...
module.exports = {
  ...
  async execute(ctx) {
    ...
    try {
      if (arg === 'last') {
        const polls = await storage.findPollsByChat(ctx.info.from || ctx.message.from);
        ...
        Object.values(poll.votes || {}).forEach(v => {
          (v.selectedIndexes || []).forEach(i => (counts[i] = (counts[i] || 0) + 1));
        });
        let text = `Resultados (última enquete): ${poll.title || poll.name || ''}\n\n`;
        const opts = (poll.pollOptions && poll.pollOptions.map(o => o.name)) || poll.options || [];
        for (let i = 0; i < opts.length; i++) text += `${i + 1}) ${opts[i]} — ${counts[i] || 0}\n`;
      ...
      logger.error('poll.results error', err && err.message);
