/**
 * Flow: resumo de livros por período (GET /api/books/period-stats + cartão PNG).
 * Só envia imagem se summary.hasActivityInPeriod (como filmes / notas Spotify).
 */
...
const bookFlow = createFlow("books", {
  root: {
    title: "📊 Resumo de livros — escolha o período",
    options: [
      {
        label: "Esse mês",
        action: "exec",
    ...
    bookStats: async (ctx, data) => {
      try {
        ...
        if (!json.summary.hasActivityInPeriod) {
          await ctx.reply("_Nenhuma atividade de leitura neste período._");
          return { end: false, noRender: true };
        }
        ...
        const img = await renderBooksCard(templateData, {
          width: 1080,
          height: 1920,
        });
