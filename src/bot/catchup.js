...
async function runCatchup(client, options = {}) {
  ...
  for (const chat of chats) {
    try {
      const chatId =
        chat.id && chat.id._serialized ? chat.id._serialized : chat.id || chat.name || 'unknown';
      const lastTs = storage.getLastTs(chatId) || 0;
      // Buscar últimas mensagens (limit)
      const messages = await chat.fetchMessages({ limit: Math.min(limitPerChat, 200) });
      ...
      for (const msg of newMsgs) {
        try {
          ...
          if (msg.id && msg.id._serialized) storage.markProcessed(msg.id._serialized);
      ...
      logger.warn(`Catchup: erro no chat ${chat.id && chat.id._serialized}: ${err && err.message}`);
    }
  }
